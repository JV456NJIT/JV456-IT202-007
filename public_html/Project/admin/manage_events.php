...
function insert_breeds_into_db($db, $events, $mappings)
{
    // Prepare SQL query
    $query = "INSERT INTO `events` ";
    if (count($events) > 0) {
        $cols = array_keys($events[0]);
        ...
        $values = [];
        foreach ($events as $i => $description) {
            $descriptionPlaceholders = array_map(function ($v) use ($i) {
                return ":" . $v . $i;  // Append the index to make each placeholder unique
            }, $cols);
            $values[] = "(" . implode(",", $descriptionPlaceholders) . ")";
        }
        ...
        foreach ($events as $i => $description) {
            foreach ($cols as $col) {
                $placeholder = ":$col$i";
                $val = isset($breed[$col]) ? $description[$col] : "";
                $param = PDO::PARAM_STR;
